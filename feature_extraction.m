...
%This function selects FEATURE_NUM features from the input data set
    ...
    for current_interval = 1:ranges_num
        ...
        extraction = [extraction max(input(start_index:end_index))]; % max
        extraction = [extraction min(input(start_index:end_index))]; % max
        extraction = [extraction median(input(start_index:end_index))]; % median
        extraction = [extraction mode(input(start_index:end_index))]; %moda
